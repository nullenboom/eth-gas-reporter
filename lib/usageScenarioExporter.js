...
class UsageScenarioExporter {
  ...
  export(data) {
    ...
    this.exportJson.metaData = this._createMetaData(data);
    this._writeIntoJsonFile(fileName);
  }

  _writeIntoJsonFile(fileName) {
    var dir = "./scenario-gas-reporter-reports";
  ...
  _createMethodData(variant) {
    ...
    _.forEach(variant.methods, (data, methodId) => {
      ...
      stats.gasUsedValues = data.gasData;
