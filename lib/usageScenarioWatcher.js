...
class UsageScenarioWatcher {
  ...
  //method to reset data for each method call
  //could be done better and should be in the future
  resetData() {
    Object.keys(this.data.methods).forEach(key => {
      this.data.methods[key].numberOfCalls = 0;
      this.data.methods[key].gasData = [];
    });
  }
