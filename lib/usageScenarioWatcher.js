...
const UsageScenarioData = require("./usageScenarioData");
...
class UsageScenarioWatcher {
  constructor(config) {
    ...
    this.data = new UsageScenarioData();
  ...
  _collectDeploymentsData(transaction, receipt) {
    ...
    if (variantMatch) {
      ...
      if (match) {
        ...
        deploymentInfo.gasData.push(utils.gas(receipt.gasUsed));
        //console.log(variantMatch.deployments)
        variantMatch.deployments.push(deploymentInfo);
        //console.log(variantMatch);
      }
    }
    // console.log(this.data.usageScenarios);
  }
  ...
  _collectMethodsData(transaction, receipt) {
    ...
    if (variantMatch) {
      ...
      if (variantMatch.methods[id]) {
        variantMatch.methods.gasData.push(utils.gas(receipt.gasUsed));
...
module.exports = UsageScenarioWatcher;
